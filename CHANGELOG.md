...
### Added
- **Track B shipped (external repo)**: the `doc-rock-studio` plugin landed in `docrock/docrock-marketplace` — a `studio` conductor (routes content requests; runs the Pulse → script → record → metadata → promo pipeline; "content sprint" reads this dashboard's scored `digest.json` directly) and a `script-writer` (personal-lens interview, no-fabrication rule, production cues, mandatory copy-audit pass). Doc's standing order is enforced throughout: all public-facing copy goes through the copy-doctor plugin. Track C's script-writer shipped with it; the copywriter overhaul was superseded by copy-doctor itself.
